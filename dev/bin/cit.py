...
def check_header_revision():
    ...
    if local_revision <= remote_revision:
        utils.rip(f"Header revision not increased (local={local_revision}, main={remote_revision}). Please bump RAPID_IMAGE_HEADER_REVISION.")
    else:
...
ap.add_argument("-t", action="store_true", help="Run test only7. Skip lint.")
ap.add_argument("test_args", nargs="*")
args = ap.parse_args()

if not args.t: check_header_revision(); run_style_check()
if not args.l: utils.run_the_latest_binary("dev/test/{variant}/rapid-image-test", args.test_args)
